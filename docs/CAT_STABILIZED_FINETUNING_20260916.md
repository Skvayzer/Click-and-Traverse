...
The first scheduled validation completed at **26,214,400** transitions in 81.3 seconds and passed every retention gate. The single best checkpoint was written from this evaluation. Deterministic CAT success was **55.208%** (baseline 52.604%), clutter **7.8125%** (baseline 9.375%), and falls zero. Stochastic CAT success was 55.729%, clutter 9.375%, with one fall in 256 episodes. Thus CAT retention passed this early benchmark, while clutter improvement is not established: deterministic clutter lost one successful episode and stochastic clutter gained one. These small counts do not establish a reliable learning trend.

At the subsequent verification, the learner had completed **29,360,128** transitions and continued running. GPU utilization was 100% during the sampled update, with 37,608 MiB used and 11,025 MiB free. Upper noise averaged 0.0485, stayed within its cap, and no non-finite training metrics were recorded. Full first-validation summaries, checkpoint rules and this runtime snapshot are in `docs/assets/cat-stabilized-20260916/first-validation.json`.
