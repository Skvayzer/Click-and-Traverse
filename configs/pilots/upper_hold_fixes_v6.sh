#!/usr/bin/env bash
# Prepared only; NOT LAUNCHED. Fixed-arm CPU hold still needs residual-reference control.
set -euo pipefail
cd "$(dirname "$0")/../.."
exec .venv-mjlab/bin/python train_cat_mjlab.py run \
  ...
  --upper-action-scale left_shoulder_pitch_joint=1.0 \
  --upper-action-scale right_shoulder_pitch_joint=1.0 \
  ...
  --run-dir outputs/cat_upper_hold_fixes_v6_20260922 \
