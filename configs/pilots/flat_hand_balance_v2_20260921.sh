#!/usr/bin/env bash
# Prepared only. Launch after the existing GPU jobs finish.
set -euo pipefail
cd "$(dirname "$0")/../.."
...
exec .venv-mjlab/bin/python train_cat_mjlab.py run \
  --checkpoint-native outputs/cat_flat_balance_ppo_37632_20260920/best.pt --fresh-optimizer \
  ...
  --require-hand-contrast --run-dir outputs/cat_flat_hand_balance_ppo_30720_20260921 \
