#!/usr/bin/env bash
# Prepared only. Exact v2-to-v3 weight transfer; bounded IK is certified at startup.
set -euo pipefail
cd "$(dirname "$0")/../.."
exec .venv-mjlab/bin/python train_cat_mjlab.py run \
  --checkpoint-native outputs/checkpoint_upgrade_v6_20260922/upgraded.pt \
  ...
  --run-dir outputs/cat_hand_reward_floor_v6_transfer_pilot50_20260922 \
