#!/usr/bin/env bash
# Prepared only; do not run alongside cat_rec. No launch performed.
set -euo pipefail
cd "$(dirname "$0")/../.."
exec .venv-mjlab/bin/python train_cat_mjlab.py run \
  --checkpoint-native outputs/cat_recover_23_30720_20260922/best.pt \
  ...
  --run-dir outputs/cat_lateral_corridor_pilot50_20260922 \
  ...
  --lateral-corridor --upper-gravity-compensation --protected-hand-sdf-margin \
  --hand-reward-soft-floor 0.2 --hand-contrast-approach-distance 0.40 \
  --hand-raised-reset-fraction 0 \
