...
# No stop/kill commands: the existing flat-balance run remains under user control.
set -euo pipefail
cd "$(dirname "$0")/../.."
exec .venv-mjlab/bin/python train_cat_mjlab.py run \
  --checkpoint-native outputs/cat_flat_balance_v1_30720_20260920/resume.pt \
  --fresh-optimizer \
  ...
  --run-dir outputs/cat_flat_balance_v2_30720_20260920 \
