#!/usr/bin/env python3
"""Rescore saved poses only. No dynamics, policy evaluation, bank writes or training."""
import os
...
OUT=ROOT/'outputs/lateral_corridor_cpu'
MARGIN=.07899
...
def main():
    ...
    def evaluate(idx,q,h,p,p0,mode,baseline=None):
        ...
        row=dict(index=idx,mode=mode,box=stats(baseline,p,g['core_start_m'],g['core_end_m'],p0),
            ...
            if mode=='absolute':
                ...
                for heading_deg in (0,90):
                    ...
                    static_nominal.append(dict(index=idx,heading_deg=heading_deg,lateral_hand_m=yy[0].tolist(),
                         ...
                         min_arm_lateral_clearance_m=float(ac.min())))
    ...
    for mode,arrays in pooled.items():
        ...
        aggregate[mode]=dict(scenes=len(rs),lateral_route_signed_quantiles_m=quant(y),lateral_root_signed_quantiles_m=quant(local),
            ...
            metrics={k:dict(core_steps=sum(r[k]['core_steps'] for r in rs),core_good_steps=sum(r[k]['core_good_steps'] for r in rs),
                ...
                mean_full_core_distance_fraction=float(np.mean([r[k]['compliant_full_core_distance_fraction'] for r in rs]))) for k in ('box','corridor','exact_original_margin_corridor','zero_margin_corridor','fk_corridor')})
    assert not torch.cuda.is_initialized()
    report=dict(scope='Saved first-core trajectories, 20-ms endpoints only; no resimulation. Baseline four unmodified-policy scenes are seeded and from an older frozen snapshot, not the current live policy. Nominal-arm witnesses are kinematic, not dynamic rollouts.',
