#!/usr/bin/env python3
"""CPU-only independent v5 copies; feasible protected hand target."""
import os
...
from cat_ppo.furniture.balanced_region_bank import SCHEMA, feasible_region
...
def build():
    old = ROOT/'data/furniture/cat_flat_hand_balance_v3_20260921'
    new = old.with_name('cat_flat_hand_balance_v5_20260921')
    ...
    protected = source + [ROOT/('data/furniture/cat_flat_hand_balance_v2_20260921'+s) for s in suffixes]
    protected += [ROOT/('data/furniture/cat_flat_hand_balance_v4_20260921'+s) for s in suffixes]
    before_all = [snapshot(p) for p in protected]
    before = before_all[:3]
    required = sum(v['size'] for snap in before for v in snap.values())
    assert shutil.disk_usage(ROOT).free-required > 2*1024**3, 'Insufficient disk headroom'
    write(ROOT/'outputs/raised_region_v5_20260921/source_before.json', dict(zip(map(str,protected),before_all)))
    ...
    for record in manifest['scenes']:
        ...
        revised = feasible_region(original)
        write(path, revised)
    ...
    manifest['flat_balance'].update(schema='cat-protected-region-v5', region_geometry_source=dict(
    ...
    after_all = [snapshot(p) for p in protected]
    assert before_all == after_all, 'Source bytes or inode/mtime changed'
    differences = []
    ...
    report = dict(preflight_v3=results[0], preflight_v5=results[1], changed_zones=changes,
        source_unchanged=True, source_file_counts=[len(s) for s in before], changed_files=differences,
        protected_file_counts=[len(s) for s in before_all], free_disk_bytes=shutil.disk_usage(ROOT).free,
        source_manifest_sha256=sha256(old/'manifest.json'), v5_manifest_sha256=sha256(new/'manifest.json'))
    write(ROOT/'outputs/raised_region_v5_20260921/verification.json', report)
