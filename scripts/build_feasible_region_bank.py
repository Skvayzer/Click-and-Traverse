#!/usr/bin/env python3
"""CPU-only independent v4 copies; feasible protected hand target."""
import os
...
from cat_ppo.furniture.feasible_region_bank import SCHEMA, feasible_region
...
def build():
    ...
    new = old.with_name('cat_flat_hand_balance_v4_20260921')
    ...
    write(ROOT/'outputs/raised_region_v4_20260921/source_before.json', dict(zip(map(str,protected),before_all)))
    ...
    manifest['flat_balance'].update(schema='cat-protected-region-v4', region_geometry_source=dict(
    ...
    report = dict(preflight_v3=results[0], preflight_v4=results[1], changed_zones=changes,
        ...
        source_manifest_sha256=sha256(old/'manifest.json'), v4_manifest_sha256=sha256(new/'manifest.json'))
    write(ROOT/'outputs/raised_region_v4_20260921/verification.json', report)
